...
TASK_SCRIPTS=( 'run-cola.sh' 'run-sst2.sh' 'run-mrpc.sh' 'run-stsb.sh' 'run-qqp.sh' 'run-mnli.sh' 'run-qnli.sh' 'run-qqp.sh' 'run-rte.sh' 'run-wnli.sh')
TASK_IDX=$1
RUN_NAME=$2
...
TASK_SCRIPT=${TASK_SCRIPT[$TASK_IDX]}
for (( i=0; i<=7; i+=1 )); do
